...
**If VERDICT: FAIL** — Follow the Review Feedback Protocol below.
...
2. Create or append to `research_review_memo.md` in the project folder with structured feedback:
   ...
   ```
3. Invoke the Worker: "Read `research_review_memo.md`. Fix all `[FIX]` items. Mark each fixed."
4. After Worker completes fixes, re-invoke Reviewer for a SECOND pass (max 2 passes total).
5. After 2 review passes, log any remaining issues as known limitations and finalize.
...
1. Read `/memories/session/research_reflections.md` (if it exists).
2. Identify patterns that appeared 3+ times (e.g., "Semantic Scholar API was consistently
   the best academic source" or "site:arxiv.org queries consistently outperformed broad searches").
3. Promote recurring patterns to `/memories/repo/research_patterns.md` using the memory tool.
4. These patterns persist across sessions and inform future Worker behavior.
